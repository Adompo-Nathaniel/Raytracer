...
Parallelepipede::Parallelepipede(const Vec3D& c, float width, float height, float depth, const Material& m)
    : center(c), w(width), h(height), d(depth), material(m) {}

bool Parallelepipede::hit(const Rayon3D& r, float t_min, float t_max, Intersection3D& rec) const {
    float half_w = w / 2.0f;
    float half_h = h / 2.0f;
    float half_d = d / 2.0f;
